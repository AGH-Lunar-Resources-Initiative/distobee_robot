...
from distobee_bringup.utils import get_arg_decls, is_distobee_composable

# If type checking, import the right TypedDict instead of using regular dict
from typing import TYPE_CHECKING, Literal

if TYPE_CHECKING:
    from distobee_bringup.type_hints import BringupConfig
else:
    BringupConfig = dict

COMPONENT_CONTAINER_NAME = "distobee_container"
...
def gen_launch(
    ...
    for module_name, args in config.items():
        if module_name not in installed_packages:
            module_pkg = f"distobee_{module_name}"
        else:
        ...
        if composition != "none" and is_distobee_composable(launch_path):
